...
const $btnagregar_destinatario = $("#btnagregar_destinatario")
...
//Configuraciones
    ...
    $("#frmRegistrar").validate({
        ...
        invalidHandler: function(form, validator) {
            ...
            if(errors > 0)
            {
              if(validator.errorList[0].element.id == 'idcliente'){
...
function preinicio()
{
   if ($("#descripciongeneral").val() == '')
   {
       ...
       $("#nuevadescripciongeneral").attr('required', 'true');
  ...
  if($('#idremitente').val() == $('#idcliente').val())
  {
      $("#cbox1").attr("checked" , true);
      $('#idremitente').attr("disabled", true);
 ...
 $("#addrow").click( function() {
  ...
  if($('#idremitente').val() == $('#ddlcliente').val())
  {
      $("#cbox1").attr("checked" , true);
      $('#idremitente').attr("disabled", true);

  }
  if($('#iddestinatario').val() == $('#ddlcliente').val())
  {
      $("#cbox2").attr("checked" , true);
      $('#iddestinatario').attr("disabled", true);
      $('#iddestinatario').trigger("chosen:updated");
...
function BeginDropDownlistCliente()
{
   $("#idcliente").change(function ()
   {
      ...
      if($('#cbox2').is(':checked')==true)
      {
             ...
             $("#iddestinatario").prop( "disabled", true );
             $('#iddestinatario').trigger("chosen:updated");
             $("#iddestinatario_selected").val($("#idcliente").val());
      }
...
function Recalcular()
{
  let formula = $('#idformula').val();
  let idorigen = $('#idorigen').val();
  let iddestino = $('#iddestino').val();
  let idcliente = $('#idcliente').val();
  let idtipotransporte = $('#idtipotransporte').val();
  let pesogeneral = $('#pesogeneral').val();
  let volgeneral = $('#volgeneral').val();
  let bultogeneral = $('#bultogeneral').val();
  let pesovol = $('#pesovol').val();

  if(formula == "")
  {
     swal({ title: "¡No se puede calcular!", text: "¡Debe seleccionar una fórmula!", type: "warning", confirmButtonText: "Aceptar" });
     return ;
  }
  if(idorigen == "")
  {
     swal({ title: "¡No se puede calcular!", text: "¡Debe ingresar un origen!", type: "warning", confirmButtonText: "Aceptar" });
     return ;
  }
  if(iddestino == "")
  {
     swal({ title: "¡No se puede calcular!", text: "¡Debe ingresar un destino!", type: "warning", confirmButtonText: "Aceptar" });
     return ;
  }
  if(idcliente == "")
  {
     swal({ title: "¡No se puede calcular!", text: "¡Debe ingresar un cliente!", type: "warning", confirmButtonText: "Aceptar" });
     return ;
  }
  if(idtipotransporte == "")
  {
     swal({ title: "¡No se puede calcular!", text: "¡Debe ingresar un tipo transporte!", type: "warning", confirmButtonText: "Aceptar" });
     return ;
  }
  if(pesogeneral == ""  && volgeneral == "" )
   {
    ...
    volgeneral = 0;
    bultogeneral = 0;
    const grid = $('#gridguias');
    let rows = grid.jqGrid('getDataIDs');
        for (i = 0; i < rows.length; i++)
        {
            let rowData = grid.jqGrid('getRowData', rows[i]);
            if(rowData['peso']!="")
            pesogeneral  = pesogeneral +  parseFloat(rowData['peso']);
            if(rowData['volumen']!="")
            volgeneral  = volgeneral + parseFloat(rowData['volumen']);
  ...
  {
     swal({ title: "¡No se puede calcular!", text: "¡Debe ingresar un peso general!", type: "warning", confirmButtonText: "Aceptar" });
     return ;
  }
  if(volgeneral.toString() == "")
  {
     swal({ title: "¡No se puede calcular!", text: "¡Debe ingresar un volumen general!", type: "warning", confirmButtonText: "Aceptar" });
     return ;
  }
